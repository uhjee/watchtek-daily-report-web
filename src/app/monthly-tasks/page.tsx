...
function mergeDuplicateTasks(tasks: DailyReport[]): DailyReport[] {
  ...
  tasks.forEach((task) => {
    // 중복 판단 키 생성
    const key = `${task.group}|${task.person}|${task.pmsNumber || ''}|${
      task.title
    }`;
...
export default function MonthlyTasksPage() {
  ...
  // 2차 필터: 선택된 group (파이차트 클릭 또는 업무구분 Select)
  ...
  const dateRangeText = useMemo(() => {
    if (selectedWeek !== ALL_WEEKS) {
      // 특정 주차 선택 시: 월요일 ~ 금요일
      const weekInfo = weekOptions.find(
      ...
      if (weekInfo) {
        ...
        const fridayStr = `${endDate.getFullYear()}-${String(
          endDate.getMonth() + 1,
        ).padStart(2, '0')}-${String(endDate.getDate()).padStart(2, '0')}`;
        return `${formatDateToShortFormat(
          weekInfo.startDate,
        )} ~ ${formatDateToShortFormat(fridayStr)}`;
      }
    }
    // 전체 주차 선택 시 (월 전체 범위: 1일 ~ 말일)
    const firstDay = `${selectedYear}-${String(selectedMonth).padStart(
      2,
      '0',
    )}-01`;
    const lastDayDate = new Date(selectedYear, selectedMonth, 0);
    const lastDay = `${selectedYear}-${String(selectedMonth).padStart(
      2,
      '0',
    )}-${String(lastDayDate.getDate()).padStart(2, '0')}`;
    return `${formatDateToShortFormat(firstDay)} ~ ${formatDateToShortFormat(
      lastDay,
    )}`;
  ...
  const baseFilteredTasks = useMemo(() => {
    ...
    const filteredTasks = data.tasks.filter((task) => {
      ...
      if (selectedMember !== ALL_MEMBERS && task.person !== selectedMember)
        return false;
      ...
      if (selectedWeekInfo) {
        ...
        if (
          completionDate < selectedWeekInfo.startDate ||
          completionDate > selectedWeekInfo.endDate
        ) {
          return false;
        }
      } else {
  ...
  const sortedTasks = useMemo(() => {
    ...
    if (excludeMeeting) {
      tasks = tasks.filter(
        (task) => task.group !== '회의' && !task.title.includes('회의'),
      );
    }
  ...
  const totalManDays = useMemo(() => {
    const totalManHours = sortedTasks.reduce(
      (sum, task) => sum + task.manHour,
      0,
    );
  ...
  const leaveItems = useMemo((): LeaveItem[] => {
    ...
    // 주차 필터 적용
    if (selectedWeekInfo) {
      return allLeaveItems.filter(
        (item) =>
          item.date >= selectedWeekInfo.startDate &&
          item.date <= selectedWeekInfo.endDate,
      );
    }
  ...
  const handleExcelDownload = async () => {
    ...
    const memberTasksMap = members.map((member) => {
      const filteredTasks = data.tasks.filter((task) => {
        ...
        // 업무구분 필터 적용
        if (selectedGroup && task.group !== selectedGroup) return false;

        // 회의 제외 필터 적용
        if (excludeMeeting) {
          if (task.group === '회의' || task.title.includes('회의')) return false;
        }

        // 진척도 필터 적용
        if (progressFilter === 'completed' && task.progressRate !== 100)
          return false;
        if (progressFilter === 'in_progress' && task.progressRate === 100)
          return false;
    ...
    await downloadMonthlyTasksExcel(
      selectedYear,
      selectedMonth,
      memberTasksMap,
    );
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setProgressFilter(
                        value as 'all' | 'completed' | 'in_progress',
                      )
                    }
                  >
                ...
                <div className="flex items-center gap-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setExcludeMeeting(checked === true)
                    }
                  />
                  <Label
                  ...
                  {sortedTasks.map((task, index) => {
                    ...
                    return (
                      <TableRow key={`${task.id}-${task.person}`}>
                        ...
                        <TableCell
                          className="truncate cursor-pointer hover:text-primary transition-colors group/title"
                          title={task.title}
                          onClick={() => handleCopyTitle(task.title)}
                        >
                          <span className="flex items-center gap-1">
                            <span className="truncate">{task.title}</span>
                            <Copy className="w-3 h-3 opacity-0 group-hover/title:opacity-50 flex-shrink-0 transition-opacity" />
                          </span>
                        </TableCell>
                        <TableCell>-</TableCell>
